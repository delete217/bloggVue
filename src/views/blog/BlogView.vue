      ...
      <el-main>
        ...
        <div class="me-view-card">
          ...
          <div class="me-view-tag">
            ...
            <el-button @click="tagOrCategory('category', article.categoryVo.id)" size="mini" type="primary"  :c ="article.categoryVo" round plain>{{article.categoryVo.categoryName}}</el-button>
          </div>
       ...
       <!-- <item v-for="(c,index) in comments" :key="c.id" :index = "index" :comment = "c" :rootCommentCounts="comments.length" v-bind="c"></item> -->
...
<script>
  ...
  import ItemVue from '../../components/comment/Item.vue'
  ...
  export default {
    ...
    computed: {
      ...
      title() {
        return `${this.article.title} - 文章 - 1989 OnLine`
      }
    },
    methods: {
      tagOrCategory(type, id) {
        this.$router.push({path: `/${type}/details/${id}`})
      },
      editArticle() {
      ...
      getArticle() {
        ...
        viewArticle(that.$route.params.id).then(data => {
          ...
          that.article.editor.value = data.data.bodyVo.content
      ...
      getCommentsByArticle() {
        ...
        getCommentsByArticle(that.article.id).then(data => {
          if(data.success){
               ...
               this.article.commentCounts = that.comments.length
          }else{
